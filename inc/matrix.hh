...
private:
  ...
  // Returns blkock offset within a row --- see get_idx
...
public:
   ...
   */
  void collect_column_indices(unsigned col,
   ...
   * @param indices Vector to store row indices where bits are set
   */
  void collect_column_indices_fast(unsigned col,
                                   std::vector<unsigned> &indices) const {
    if (col >= _cols) {
      return;
    }
  ...
  bool is_bit_set_in_row_data(const std::vector<avx_block> &row_data,
                              unsigned col) const {
    if (col >= _cols) {
      return false;
    }
   ...
   */
  void row_masked_or(unsigned target_row, unsigned source_row,
   ...
   * performance
   */
  void apply_row_mask(unsigned target_row,
   ...
   */
  bool apply_row_mask_efficient(unsigned target_row,
                                const std::vector<avx_block> &source_data) {
    if (target_row >= _rows || source_data.size() < _width) {
      return false;
    }
