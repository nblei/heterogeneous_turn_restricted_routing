...
struct PerformanceStats {
  ...
  void reset() {
    ...
    calculate_edge_priority_time =
        std::chrono::duration<int64_t, std::micro>{0};
    optimize_additional_edges_time =
        std::chrono::duration<int64_t, std::micro>{0};
...
public:
  ...
  void print_perf_stats() const { _perf_stats.print(); }

  const PerformanceStats &get_perf_stats() const { return _perf_stats; }

  void reset_perf_stats() { _perf_stats.reset(); }
  ...
  bool optimize_random() {
    ...
    do {
      ...
      // for (unsigned i = 0; i < graph._n && !added_any; i++) {
      //   for (unsigned j = 0; j < graph._n && !added_any; j++) {
      ...
      //           // Print progress every 100 edges added
      //           if (total_edges_added % 100 == 0) {
      //             std::cout << "Added " << total_edges_added
      //                       << " edges so far...\n";
      //           }
      //         }
      //         break;
