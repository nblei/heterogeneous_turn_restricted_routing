...
void print_usage(const char *program_name) {
  std::cout
      << "Usage: " << program_name
      << " rows cols routing_type [--optimize|--opt-random] [--log filename] "
         "[--booksim filename]\n"
      ...
      << "  --optimize: Optional flag to apply routing optimizer "
         "(priority-based)\n"
      << "  --opt-random: Optional flag to apply random edge selection "
         "optimizer\n"
      << "  --log filename: Optional flag to log turn edges to specified file\n"
      << "  --booksim filename: Optional flag to export network to BookSim2 "
         "anynet format\n";
}
...
int main(int argc, char *argv[]) {
  ...
  bool optimize_random_flag = false;
  ...
  for (int i = 4; i < argc; i++) {
    ...
    if (arg == "--optimize") {
      optimize_flag = true;
    } else if (arg == "--opt-random") {
      optimize_random_flag = true;
  ...
  if (optimize_flag || optimize_random_flag) {
    if (optimize_flag) {
      std::cout << "\nApplying priority-based routing optimization...\n";
    } else {
      std::cout << "\nApplying random routing optimization...\n";
    }
  ...
  if (!log_filename.empty()) {
    ...
    bool optimized = optimize_flag || optimize_random_flag;
    if (d.log_turn_edges(log_filename, routing_type, optimized)) {
  ...
  if (!booksim_filename.empty()) {
    std::cout << "\nExporting network to BookSim2 anynet format: "
              << booksim_filename << "\n";
    bool optimized = optimize_flag || optimize_random_flag;
    if (d.export_booksim2_anynet(booksim_filename, routing_type, optimized)) {
